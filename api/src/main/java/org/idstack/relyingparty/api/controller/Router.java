...
public class Router {
    ...
    protected String evaluateDocuments(FeatureImpl feature, String storeFilePath, MultipartHttpServletRequest request, String json, String email, String tmpFilePath) {
        JsonArray jsonList = new JsonParser().parse(json).getAsJsonObject().get(Constant.JSON_LIST).getAsJsonArray();
        ...
        for (int i = 1; i <= jsonList.size(); i++) {
            ...
            try {
                ...
                String pdfPath = feature.storeDocuments(pdf.getBytes(), storeFilePath, email, request.getParameter(Constant.DOCUMENT_TYPE + i), Constant.FileExtenstion.PDF, uuid, i);
            ...
            String jsonString  = feature.storeDocuments(doc.toString().getBytes(), storeFilePath, email, metaData.getDocumentType(), Constant.FileExtenstion.JSON, uuid, i);
        }
